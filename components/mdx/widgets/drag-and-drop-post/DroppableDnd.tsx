...
function Droppable(props: DroppableProps) {
  ...
  return (
    <div
      ref={setNodeRef}
      className={cx(
        "flex  bg-shark-800 p-2 w-full max-w-[232px] min-h-[64px]  h-full gap-2 items-start justify-start z-10 rounded-md shadow-border-shiny",
        isOver && "bg-shark-700",
        className
      )}
    >
...
function Draggable(props: DraggableProps) {
  ...
  return (
    <div
      ...
      style={{ ...style, ...styles }}
      className={cx(
        "flex items-center bg-shark-800 justify-center gap-1 h-12 w-12 p-1.5 rounded-md text-silver shadow-border-shiny transition-colors cursor-grab active:cursor-grabbing z-10 drop-shadow-lg"
      )}
      {...listeners}
      {...attributes}
    >
