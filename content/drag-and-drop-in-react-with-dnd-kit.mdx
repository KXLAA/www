...
- `setNodeRef`: a callback ref we attach to the HTML element we want to make draggable (the div in the code above). This is essential for providing dndkit with the element that needs to be dragged.
...
The state of our `Draggable` components is represented in state as an array of objects. Each object contains an ID, name, and the current position of the `Draggable` component. We pass the current positions in state to the `style` object's `left` and `top` CSS properties with an `absolute` position, allowing the component to be dragged across the whole area.

We need a way to update the position of our `Draggable` component as a user drags it. We can leverage one of the events emitted by the `DndContext` - `onDragEnd` - to do this.

The `onDragEnd` event is triggered at the end of a drag event. It contains lots of information about the active `Draggable` component, including its position, ID, and more. We can access this data with a handler function and use it to update the `Draggable` components' positions in state:
...
<Callout intent='default' dropdown heading='Code Breakdown'>
For this interaction we need a `Draggable` & `Droppable` Component. The Draggable component needed here is identical to the one we built above so i wont repeat it here.
...
- `setNodeRef` - a callback ref we attach to the HTML element we want to make droppable. This is how dndkit knows an element is a drop area.
...
To render the overlay, we need the details of the `Draggable` that is currently being dragged. The `onDragStart` event emitted by the `DndContext` sets the state of the `activeItem` and renders an overlay with the details of the current `Draggable`:
...
Finally, we need to update the state of our `Draggable` components. To do this, we'll use the `onDragOver` event emitted by the `DndContext`. This event is triggered when a `Draggable` is dragged over a droppable area and provides information about the dragged element and the droppable area it is over.
...
The `SortableItem` component takes a unique `id` and a `name` as props. We pass the `id` we receive as props as a field of the object we pass to the `useSortable` hook; this `id` field is required.

The `useSortable` hook is similar to the `useDraggable` hook. In addition to what the `useDraggable` hook returns, it also returns:

- `isDragging` - Set to `true` when dragging a sortable component.
- `transition` - Adds CSS animations to the Sortable Component.
