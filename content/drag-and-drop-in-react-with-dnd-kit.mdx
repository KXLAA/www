...
What are drag handles? Drag handles are HTML elements that trigger a drag event. In our draggable component, we set the draggable source (i.e., the div that we attached the setNodeRef callback to) to act as the drag handle.
...
To see when the `DndContext` emits different events, try the demo below:
