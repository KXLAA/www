...
In more advanced types of drag-and-drop UI interactions, we also have sortable components. These are usually used in lists, where each list item is both a draggable component and a droppable area. When you drag a list item, it automatically sorts with the other list items:

<SortableDnd />

In some implementations of this type interaction, like Trello and Jira for example, we can sort both lists and list items, so both lists and list items are sortable components:

<SortableMultiDnd />

To handle this use case, DnD-Kit exposes a set of presets from the `@dnd-kit/sortable` package:

- The `useSortable` hook, which turns HTML elements into sortable components. This hook is a combination of the `useDraggable` and `useDroppable` hooks.
- The `SortableContext`, which manages the interaction between sortable components.

You can install the presets by running the following command:
...
We can easily create draggable components using the `useDraggable` hook. This hook takes an input object with a required `id` as an argument and returns an object with several properties. We are currently interested in four of these properties:
...
4. `transform` - An object with the x- and y-coordinates of a draggable element in 2D, which is updated during a drag event. We can use the values of the object to update the position of our draggable element with CSS using `translate()`.
...
Drag handles are HTML elements that trigger a drag event. In our draggable component, we set the draggable source (i.e., the div that we attached the setNodeRef callback to) to act as the drag handle.

However, we can make a different HTML element the drag handle. We can do this easily by attaching the listeners and attribute properties returned by the useDraggable hook to a different HTML element:
...
The `DndContext` provides many props to configure the behaviour of our draggable and droppable components. For a full API reference, please refer to the [documentation](https://docs.dndkit.com/api-documentation/context-provider). The most important props at this point are the events-related and sensors props, which I will discuss below:

#### Events

The `DndContext` emits various events throughout a drag-and-drop interaction. We can use the information from these events to configure the behavior of draggable and droppable components. The main events emitted are:
...
- `onDragCancel`, which fires if a drag operation is cancelled, e.g. when the user presses `escape` while dragging a draggable item.
...
`Dndkit` exposes the concept of sensors to detect potential input types that can trigger a drag event, such as a mouse, touchpad, keyboard, stylus pen, or touchscreen. By default, `@dnd-kit/core` ships with four sensors: `MouseSensor`, `TouchSensor`, `KeyboardSensor`, and `PointerSensor`. These are generally enough for most use cases. However, if they are not sufficient, you can build a custom sensor. For more information on this, check [here](https://docs.dndkit.com/api-documentation/sensors#custom-sensors).

`DndContext` uses the `Pointer` and `Keyboard` sensors by default. To use other sensors, such as the Mouse and Touch sensors, we’ll need to initialise them separately using the `useSensor` hook:
...
Sortable components are a combination of draggable and droppable components. The primitives needed to build these components are exposed as a preset via the `@dndkit/sortable` package. The `useSortable` hook can be used to create Sortable components; it is quite similar to the `useDraggable` hook:
...
In addition to the useSortable hook, we also have the `SortableContext`. This component manages the interaction between our sortable components. It requires that you pass it a sorted array of the unique identifiers associated with the elements that use the useSortable hook within it:
