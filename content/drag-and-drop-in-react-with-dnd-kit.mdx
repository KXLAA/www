...
The `<DndContext>` context is essential for managing interactions between draggable and droppable components, as well as emitting events at different stages of a drag and drop interaction. We can then react to these events and use the information to update the state of the draggable and droppable components in our UI. The events we can listen to are:

- `onDragStart` triggers when a draggable is dragged. It contains the DragStartEvent object which contains information about the element that has just been dragged.
- `onDragOver` triggers when a draggable item is moved over a droppable container. It contains the DragOverEvent object which contains information about the draggable item and droppable container which the draggable item was dropped over.
- `onDragEnd` fires after a draggable item is dropped. This event contains information about the active draggable `id` along with information on whether the draggable item was dropped `over`.
- `onDragCancel` fires if a drag operation is cancelled, for example, if the user presses `escape` while dragging a draggable item.

You can see these events in action by trying out the demo below:
