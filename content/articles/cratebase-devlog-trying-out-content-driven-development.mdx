...
The only downside is that it doesn't support .AIFF files. My current workaround is to convert these files to MP3 and send that as a blob for the element to play. This causes some delay when playing an AIFF track—I'm looking for a way to solve this.
...
I think the find metadata feature will be the most complex to implement—but let's see how it goes.
