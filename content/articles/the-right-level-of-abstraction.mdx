...
tRPC, an RPC library for TypeScript that uses tanstack query under the hood, has recently embraced this approach through a significant change. In previous versions, tRPC wrapped its generated client in custom versions of `useQuery` and `useMutation`. The [new version](https://trpc.io/blog/introducing-tanstack-react-query-client) now provides a function that returns an object matching the `QueryOptions` and `MutationOptions` interfaces native to tanstack query.
...
I have been playing around with [Hey API](https://heyapi.dev/) for tanstack query code generation, and it embraces a similar philosophy. Instead of generating custom tanstack query hooks, it simply generates a function that returns query options matching what `useQuery` or `useMutation` expects from tanstack query. 
