...
However, sometimes you may want to consider overriding them. This is a non-comprehensive list of times when you might want to override TanStack Query's defaults.
...
Why is this a good idea? Leaving the default retry option on for tests can add significant time to your unit or integration tests, which slows down the feedback loop—especially for larger applications. Most of these tests simply check that we are handling errors and showing error states for HTTP errors correctly. There's no value in waiting for three retries before asserting these error states.
...
You can see examples of all these approaches in open source repos [here](https://grep.app/search?q=retry:+\(failureCount\)).
